...
**Asana:** MASTER-1972 (GID `1217187690537803`) — Project MASTER → IN PROGRESS  
**Branch:** `MASTER-1972`  
...
| 4 | INTEGRATION | Dogfood on the **real** tmux server: `source ~/.bashrc` (or open a fresh shell in the worktree), `tdl c` in a throwaway dir, confirm 2 panes / ~50-50 / nvim + opencode / no bottom shell; `tdl c cx` still stacks a second AI. If red, loop back to step 2. | Opus 4.8 |
| 5 | INTEGRATION | Asana → move MASTER-1972 to REVIEW; comment with commit SHAs + dogfood result; archive or leave plan per repo convention. | GLM 5.2 |

## Testing strategy

- **Primary:** hermetic `tmux -L tdltest` + `MANUAL-VERIFICATION.md`
  checklist (RED before change, GREEN after).
- **Live dogfood (step 4):** real `tdl c` / `tdl c cx` once bashrc is
  on the active shell (`~/.bashrc` → worktree file via existing
  symlink/`sb`).
...
| Risk | Mitigation |
| --- | --- |
| `~/.bashrc` symlink may point at `~/src/dotfiles/bash/.bashrc` (main worktree), not this worktree | Confirm `readlink -f ~/.bashrc` before dogfood; if it tracks main, either merge/cherry-pick first or `source` the worktree bashrc explicitly for the test shell |
...
| Sending real `opencode`/`nvim` into the fixture hangs the server | Fixture uses `true` / `false` / `sleep` only; live apps only in step 4 |
...
- After 1 → ▶️ Step `2` of `5` — `GREEN: full tdl override in bashrc` — model: `GLM 5.2` — plan: `docs/plans/tdl-no-bottom-pane/PLAN.md`
- After 2 → ▶️ Step `3` of `5` — `REFACTOR: comment / residue cleanup` — model: `Grok Build 0.1` — plan: `docs/plans/tdl-no-bottom-pane/PLAN.md`
- After 3 → ▶️ Step `4` of `5` — `INTEGRATION: dogfood on live tmux` — model: `Opus 4.8` — plan: `docs/plans/tdl-no-bottom-pane/PLAN.md`
- After 4 → ▶️ Step `5` of `5` — `INTEGRATION: Asana → REVIEW + bookkeeping` — model: `GLM 5.2` — plan: `docs/plans/tdl-no-bottom-pane/PLAN.md`

▶️ Step `1` of `5` — `RED: pin 2-pane tdl layout expectations` — model: `GLM 5.2` — plan: `docs/plans/tdl-no-bottom-pane/PLAN.md`
