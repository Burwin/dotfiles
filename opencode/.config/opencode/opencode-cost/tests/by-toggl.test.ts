...
const TOGGL_FIXTURE: ToggleRow[] = [
  {
    worktree_path: "~/src/bamboo/acme/widgets/BAM-123",
    project_id: 1,
  ...
  {
    worktree_path: "~/src/bamboo/acme/gadgets/BAM-124",
    project_id: 2,
...
describe("§10.2 — per-client rollup", () => {
  ...
  test("with --no-model-split: 4 rows, AcmeCorp collapses to $10.10", () => {
    const { db } = buildFixture()
    try {
...
describe("§10.3 — per-project rollup", () => {
  test("with model split (default): 6 rows, ties broken by (client, project) asc", () => {
    ...
    try {
      ...
      // Gadgets and Widgets are tied at $5.00; (client, project) asc
      // puts Gadgets first.
      expect(rows[0]).toEqual({
        client: "AcmeCorp",
        project: "Gadgets",
...
describe("§10.1.d invariants", () => {
  ...
  test("4. integer-fxp8 only: cost_fxp8 stays integer through aggregation", () => {
    ...
    try {
      ...
      const acme = byClient.find((r) => r.client === "AcmeCorp")
      expect(acme?.cost_fxp8).toBe(1_010_000_000)
    } finally {
      db.close()
    }
  })
...
describe("§10.1.e edge cases", () => {
  ...
  test("c. retargeted via toggl-set mid-history: current state authoritative", () => {
    ...
    // for ses_A1's worktree from (AcmeCorp/Widgets/BAM-123) to (AcmeCorp/Widgets/BAM-999).
    ...
    const retargetedToggl: ToggleRow[] = TOGGL_FIXTURE.map((t) =>
      t.worktree_path === "~/src/bamboo/acme/widgets/BAM-123"
        ? { ...t, task: "BAM-999" }
        : t,
    )
  ...
  test("d. title-gen / subagent inherits parent's mapping via worktree JOIN", () => {
    ...
    // ses_A1 (BAM-123/Widgets/AcmeCorp), with its own zen_usage row.
...
describe("CLI surface", () => {
  ...
  test("--json output round-trips back to the AggregatedRow set", () => {
    ...
    // Spot-check: $10.00 is the largest single (AcmeCorp, claude-opus-4-7).
    expect(parsed[0].client).toBe("AcmeCorp")
